...
export const getClientNameById = async (clientId: string): Promise<ResponseType> => {
  try {
    const clientRef = doc(firestore, "clients", clientId);
    const clientDoc = await getDoc(clientRef);
    ...
    return {
      success: true,
      data: clientData.name || "Cliente"
    };
  } catch (error: any) {
    console.log("Error fetching client name:", error);
    return {
      success: false,
      msg: error?.message || "Error al obtener el nombre del cliente"
    };
  }
};