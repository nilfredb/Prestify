...
const Clients = () => {
  ...
  // Close menus when tapping outside or pressing back button
  useEffect(() => {
    const backHandler = () => {
  ...
  return (
    <ScreenWrapper style={styles.wrapper}>
      ...
      {/* Touch handler overlay for closing filter menu */}
      {showFilterOptions && (
        <TouchableOpacity
          style={styles.globalTouchHandler}
          activeOpacity={1}
        ...
        <>
          ...
          {clients.length === 0 ? (
            ...
            <FlatList
              ...
              windowSize={10}
              // Close filter if open when scrolling
              onScrollBeginDrag={() => showFilterOptions && setShowFilterOptions(false)}
            />
          )}
        </>
      )}
...
const styles = StyleSheet.create({
  ...
  filterOptionsContainer: {
    ...
    zIndex: 200,
    width: scale(150),
    // Asegurarse de que los elementos del filtro capturan toques
    pointerEvents: 'auto',
  },
  filterOption: {
    flexDirection: 'row',
